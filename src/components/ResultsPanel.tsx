...
export default function ResultsPanel({ loading, error, result, onScoreConfirmed }: ResultsPanelProps) {
  if (loading) {
    return (
      <div className="flex items-center gap-3 rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-5">
        ...
        <p className="text-sm text-gray-600 dark:text-slate-400">KI-Bewertung wird geladen…</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-surface p-5 dark:border-white/10">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-gray-400 dark:text-slate-500">KI-Bewertung</p>
          <p className={`mt-1 text-2xl font-bold ${scoreColor}`}>
            {score} / {maxPoints}{' '}
            <span className="text-base font-normal text-gray-400 dark:text-slate-500">Punkte ({pct}%)</span>
          </p>
        </div>
        <span className="text-2xl">{icon}</span>
      </div>

      <div className="rounded-lg border border-gray-100 bg-gray-50 p-4 dark:border-white/5 dark:bg-white/[0.02]">
        <p className="whitespace-pre-wrap text-sm leading-relaxed text-gray-700 dark:text-slate-300">{feedback}</p>
      </div>
